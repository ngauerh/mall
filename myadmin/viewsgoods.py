...
def orderxq(request):
    ...
    return render(request, 'myadmin/order_xg.html', context)


# 订单状态修改
def orderztxg(request, gid):
    ztxg = Orders.objects.get(id=gid)
    context = {'ztxg_list': ztxg}
    return render(request, "myadmin/users/ztxg.html", context)


# 修改状态
def ordxg(request, gid):
    try:
